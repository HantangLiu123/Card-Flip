...
module card_flip(
    ...
    wire [N_OBJ * COLOR_DEPTH - 1:0] inst_color_bus;  // COLOR_DEPTH is the num of input    
	 wire [0:47] card_nums;
	 wire [0:47] card_nums_generated;
	 wire [0:15] card_shows;
	...
	object_mem MEM9 ({YC,XC}, CLOCK_50, mem_color_bus[IDX9 * COLOR_DEPTH +: COLOR_DEPTH]);
		defparam MEM9.n = COLOR_DEPTH;
		defparam MEM9.Mn = WIDTH_BIT + HEIGHT_BIT;
		defparam MEM9.INIT_FILE = "./MIF/crown.mif";
		
	// random generator
	wire rand_busy, rand_start;
	random_start_ver Random_Generator (.clk(CLOCK_50), .resetn(Resetn), .start(rand_start), .random_num(card_nums_generated), .random_assign_busy(rand_busy));
	
	// card_num and show register controllers
	wire load_card;
	wire resetn_show, E_show, status_show;
	wire [3:0] card_show_in;
	card_num_regsiter CARD_NUM_REGISTER (.resetn(Resetn), .clk(CLOCK_50), .load(card_nums_generated), .load_enable(load_card), .new_state(card_nums));
	card_show_register SHOW_REGISTER (.resetn(resetn_show), .clk(CLOCK_50), .enable(E_show), .idx(card_show_in), .new_status(status_show), .new_map(card_shows));
	
	// timer
	wire game_start, game_end, resetn_timer;
	counter COUNTER (.game_start(game_start), .game_end(game_end), .clk(CLOCK_50), .resetn(resetn_timer),
							.HEX5(HEX5), .HEX4(HEX4), .HEX3(HEX3), .HEX2(HEX2), .HEX1(HEX1), .HEX0(HEX0));
							
	game GAME (
		.clock(CLOCK_50),
		.resetn(Resetn),
		.game_start(main_start),
		.rand_busy(rand_busy),
		.mouse_x(mouse_x),
		.mouse_y(mouse_y),
		.left_button(left_button),
		.show_busy(show_busy),
		.card_num_reg(card_nums),
		.show_reg(card_shows),
		.rand_start(rand_start),
		.load_card(load_card),
		.resetn_show(resetn_show),
		.resetn_timer(resetn_timer),
		.update_start(keypressed),
		.timer_start(game_start),
		.timer_end(game_end),
		.E_show(E_show),
		.status_show(status_show),
		.card_show(card_show_in)
	);
